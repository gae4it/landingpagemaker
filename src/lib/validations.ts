...
export const sectionSchema = z.object({
  ...
  buttons: z.array(buttonSchema).max(MAX_BUTTONS_PER_SECTION, `Maximum ${MAX_BUTTONS_PER_SECTION} buttons per section`).optional(),
  images: z.array(imageSchema).max(MAX_IMAGES_PER_SECTION, `Maximum ${MAX_IMAGES_PER_SECTION} images per section`).optional(),
});
...
export const landingPageSchema = z.object({
  ...
  sections: z.array(sectionSchema).max(MAX_SECTIONS_PER_PAGE, `Maximum ${MAX_SECTIONS_PER_PAGE} sections per landing page`).optional(),
});
